  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-black text-slate-900">Gym Membership Packages</h2>
          <p className="text-xs text-slate-500">Manage billing duration and fee structures for members</p>
        </div>
        <button
          onClick={() => setIsAddModalOpen(true)}
          className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-400 hover:to-teal-400 text-white font-bold text-xs rounded-xl shadow-sm shadow-cyan-500/20 transition-all flex items-center space-x-1"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((p) => (
            <div
              key={p._id}
              className="p-6 rounded-3xl bg-white border border-slate-200/80 hover:border-cyan-300 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
