...
import { trainersAPI, authAPI } from '../../services/api';
import { PhotoAvatarSelector } from '../common/PhotoAvatarSelector';

export const TrainerProfileModal = ({ isOpen, onClose, trainerUser, onTrainerUpdated }) => {
  ...
  const isFirstLogin = trainerUser?.isFirstLogin !== false;

  useEffect(() => {
    if (trainerUser && isOpen) {
      setUserName(trainerUser.userName || '');
      setPhone(trainerUser.phone || '');
      setSpecialty(trainerUser.specialty || 'Fitness Coach');
      setProfilePic(trainerUser.profilePic || '');
      setCurrentPassword('');
      setOtpCode('');
      setDevOtpHint('');
      setOtpSent(false);
  ...
  }, [trainerUser, isOpen, isFirstLogin]);
  ...
  const handleUpdateProfile = async (e) => {
    ...
    const trainerId = trainerUser.id || trainerUser._id;
    const res = await trainersAPI.updateProfile(trainerId, {
    ...
    if (res.success && res.data) {
      ...
      const updated = { ...trainerUser, ...res.data };
      localStorage.setItem('gym_app_user', JSON.stringify(updated));
      if (onTrainerUpdated) onTrainerUpdated(updated);
      setTimeout(() => {
        onClose();
      }, 1000);
    } else {
  ...
  const handleRequestOtp = async () => {
    ...
    const res = await authAPI.forgotPassword(trainerUser.email);
    setLoading(false);

    if (res.success) {
      setOtpSent(true);
      if (res.devOtp) setDevOtpHint(res.devOtp);
      setSuccessMsg(`OTP sent to ${trainerUser.email}. Please check your Google inbox.`);
    } else {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    if (passwordMode === 'otp') {
      ...
      if (res.success) {
        ...
        const updated = { ...trainerUser, isFirstLogin: false };
        localStorage.setItem('gym_app_user', JSON.stringify(updated));
        if (onTrainerUpdated) onTrainerUpdated(updated);
      ...
      const trainerId = trainerUser.id || trainerUser._id;
      const res = await trainersAPI.changePassword(trainerId, {
        currentPassword: currentPassword || trainerUser.userName,
        newPassword: newPassword.trim()
      });
      ...
      <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl text-slate-800 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900">Trainer Profile Settings</h3>
              <p className="text-xs text-slate-500">Update photo, coaching details & password</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {isFirstLogin && (
          <div className="mt-4 p-3.5 rounded-2xl bg-gradient-to-r from-cyan-500/10 to-teal-500/10 border border-cyan-200 text-cyan-900 text-xs flex items-start space-x-3">
            ...
            <div>
              ...
              Your default temporary password was set to your name (<strong>{trainerUser?.userName}</strong>). Please create your own secure password below.
            </div>
          </div>
        )}
        ...
        {activeTab === 'profile' && (
          <form onSubmit={handleUpdateProfile} className="mt-4 space-y-3.5 text-xs">
            <PhotoAvatarSelector
              label="Trainer Photo (Upload or Choose Avatar)"
              value={profilePic}
            ...
            {passwordMode === 'current' && (
              <div>
                ...
                <input
                  ...
                  placeholder={isFirstLogin ? `e.g. ${trainerUser?.userName}` : "Enter your current password"}
                  className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500"
                />
                {isFirstLogin && (
                  <p className="text-[10px] text-cyan-600 mt-1">
                    Tip: For first-time login, your current password is your registered name: <strong>{trainerUser?.userName}</strong>
                  </p>
                )}
              </div>
            )}
            ...
            {passwordMode === 'otp' && (
              <div className="space-y-3 p-3 bg-cyan-50/50 border border-cyan-200 rounded-2xl">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-bold text-cyan-900">Email: {trainerUser?.email}</p>
